...
def calculate_current_density_parameters(type_current=None):
    ...
    delta_t_ini_step = 5.0 # (s). Initial time at zero current density for the stabilisation of the internal states (standard value).
    delta_t_load_step = 5.0 # (s). Loading time for the step current density function, from 0 to i_step.
    delta_t_break_step = 30  # (s). Time at i_step current density for the stabilisation of the internal states.
    i_step = 10000.0 # (A.m-2). Current density for the step current density function.
...
def calculate_operating_inputs(pola_current_parameters, type_fuel_cell, voltage_zone):
    ...
    if type_fuel_cell == "manual_setup": # Setup which are not stored in "stored_operating_inputs".
        T_des = 70 + 273.15  # K. It is the desired fuel cell temperature.
        Pa_des, Pc_des = 1.2e5, 1.2e5  # Pa. It is the desired pressure of the fuel gas (at the anode/cathode).
        Sa, Sc = 3.0, 4.0  # It is the stoichiometric ratio (of hydrogen and oxygen).
        Phi_a_des, Phi_c_des = 0.1, 0.1  # It is the desired relative humidity.
        y_H2_in = 0.99 # It is the molar fraction of H2 in the dry anode gas mixture (H2/N2) injected at the inlet.
...
def calculate_physical_parameters(type_fuel_cell):
    ...
    if type_fuel_cell == "manual_setup": # Setup which are not stored in "stored_physical_parameters".
        ...
        # Global
        Aact = 25e-4  # m². It is the MEA active area.
        ...
        #   Membrane
        Hmem = 50e-6  # m. It is the thickness of the membrane.
        #   Gas diffusion layer
        ...
        i0_c_ref = 10.0  # A.m-2. It is the dry reference exchange current density at the cathode.
...
K_l_ads = 1.0
theta_l_rem = 0.0
K_v_liq_gas = 1.0    # Overall mass-transfer coefficient between liquid and gas phases (tunable)
D_liq_dif = 1e-9
